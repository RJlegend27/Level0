...
public class AnimalFarm {
	...
	AnimalFarm() {
			 ...
			 * animal.
			 */
		String farm = JOptionPane.showInputDialog(null,"What farm animal do you want");

			/* 2. Make it so that the user can keep entering new animals. */
		if(farm.equalsIgnoreCase("cow")){
			playNoise(mooFile);
		}	
		if(farm.equalsIgnoreCase("duck")){
			playNoise(quackFile);
		}
		if(farm.equalsIgnoreCase("wolf")){
			playNoise(woofFile);
		}
		if(farm.equalsIgnoreCase("llama")){
			playNoise(llamaFile);
		}
	
		if(farm.equalsIgnoreCase("cat")){
			playNoise(meowFile);
		}
	}
